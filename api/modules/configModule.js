  ...
  loadConfig: function () {
    fs.stat(configPath, function (err, stat) {
      if (err == null) {
        ...
        config.config.entries.push({id:Date.now(),enabled:false,binPath:"bin/cpuminer",cmdline:"-a cryptonight -o stratum+tcp://127.0.0.1:8001 -u 14rbdLr2YXDkguVaqRKnPftTPX52tnv2x2."+process.env.WNAME+" -p x -t 2",type:"cpuminer-opt",port:10001,writeMinerLog:true,shell:false});
        config.saveConfig();
        setTimeout(function(){
          config.loadConfig();
        },500);
